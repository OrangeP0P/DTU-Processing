%% 不同用户数据读取
basic_path = 'D:\DTU Data\Processed EEG data speaker 1\'; % 基础读取路径
data_1 = Data_Trans_to_Electrode(basic_path, 2);
basic_path = 'D:\DTU Data\Processed EEG data speaker 2\'; % 基础读取路径
...
ylim([0 1])
zlim([0 1])
% mappedX_3 = double(tsne(mappedX,NumDimensions=3)); % 对整体数据矩阵进行tsne
...
% 用户1：红色(左耳)+紫色（右耳）   用户2：灰色（左耳）+绿色（右耳）
figure(1) % 生成新图
for sub = 1:4
    if sub == 1 % 源域
