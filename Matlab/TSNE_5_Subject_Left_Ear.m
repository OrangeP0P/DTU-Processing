...
data_1 = Data_Trans_to_Electrode(basic_path, 1);
basic_path = 'D:\DTU Data\Processed EEG data left-ear\'; % 基础读取路径
data_2 = Data_Trans_to_Electrode(basic_path, 2);
basic_path = 'D:\DTU Data\Processed EEG data left-ear\'; % 基础读取路径
data_3 = Data_Trans_to_Electrode(basic_path, 3);
basic_path = 'D:\DTU Data\Processed EEG data left-ear\'; % 基础读取路径
data_4 = Data_Trans_to_Electrode(basic_path, 4);
basic_path = 'D:\DTU Data\Processed EEG data left-ear\'; % 基础读取路径
data_5 = Data_Trans_to_Electrode(basic_path, 5);

%% 调整不同数据长度
lengths = [size(data_1, 2), size(data_2, 2), size(data_3, 2), size(data_4, 2), size(data_5, 2)];
minLength = min(lengths); % 找到最短的长度
data_1 = data_1(:, 1:minLength); % 调整每个矩阵的长度
data_2 = data_2(:, 1:minLength);
data_3 = data_3(:, 1:minLength);
data_4 = data_4(:, 1:minLength);
data_5 = data_5(:, 1:minLength);

%% TSNE
all_data = [data_1; data_2; data_3; data_4; data_5]; % 合并数据集
